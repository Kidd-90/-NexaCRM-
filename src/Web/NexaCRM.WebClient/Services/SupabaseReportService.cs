...
public sealed class SupabaseReportService : IReportService
{
    ...
    public async Task SaveReportDefinitionAsync(ReportDefinition definition)
    {
    ...
    public async Task<IEnumerable<ReportDefinition>> GetReportDefinitionsAsync()
    {
        try
        {
            var client = await _clientProvider.GetClientAsync();
    ...
    public async Task<ReportData> GenerateReportAsync(ReportDefinition definition)
    {
        ArgumentNullException.ThrowIfNull(definition);
    ...
    private async Task<ReportDefinitionRecord> EnsureDefinitionRecordAsync(
    ...
    private async Task<ReportData> GetCachedReportAsync(
        string format,
        string retentionKey,
        Func<Supabase.Client, AnalyticsSourceData, Task<ReportData>> generator)
    {
        try
        {
            var client = await _clientProvider.GetClientAsync();
    ...
    private async Task<AnalyticsSourceData> LoadAnalyticsSourceDataAsync(Supabase.Client client)
    {
        ...
        await Task.WhenAll(dealsTask, stagesTask, contactsTask, ticketsTask, tasksTask, activitiesTask);
    ...
    private async Task PruneSnapshotsAsync(Supabase.Client client, Guid userId)
    {
