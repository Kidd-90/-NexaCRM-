...
public sealed class SupabaseFileHubService : IFileHubService
{
    ...
    public Task<FileDocument?> GetDocumentAsync(Guid documentId, CancellationToken cancellationToken = default)
    {
        _store.Documents.TryGetValue(documentId, out var document);
        return Task.FromResult<FileDocument?>(document);
    }
    ...
    public Task<IReadOnlyList<FileVersion>> GetDocumentVersionsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var versions = _store.DocumentVersions.TryGetValue(documentId, out var list)
            ? list
