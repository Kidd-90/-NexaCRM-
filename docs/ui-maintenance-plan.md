...
- Refactored `Pages/MainDashboard` to consume shared surface tokens, upgraded the global search to the standardized `form-control` scale, and layered motion utilities across cards, timeline, and priority buttons for consistent density + animation behaviour.
- Harmonized `Pages/SalesManagerDashboard` with the shared surface, motion, and dark theme tokens, refreshed the mobile navigation shell, and applied the new filter/button treatments to keep dashboards visually aligned.
- Modernized `Pages/StatisticsDashboardPage` with the shared skip link, form-control, and motion utilities, retokenized its scoped CSS to remove hard-coded colors, and extended dark theme coverage across tiles and charts for consistent surfaces.
